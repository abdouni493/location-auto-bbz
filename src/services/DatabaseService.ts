...
export class DatabaseService {
  ...
  private static mapWorkerRow(data: any): Worker {
    return {
      id: data.id,
  ...
  static async createWorker(
    ...
    if (rpcError && !rpcMissing) {
      console.error('[DatabaseService] create_worker_account RPC error:', rpcError);
      throw rpcError;
    }
    ...
    if (!rpcError) {
      if (result && result.success === false) {
        ...
        if (reason.includes('EMAIL_ALREADY_EXISTS')) {
