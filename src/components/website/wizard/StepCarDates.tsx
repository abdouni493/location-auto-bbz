...
export const StepCarDates: React.FC = () => {
  const {
    ...
    blockedRanges, loadingBlocked, days, promo, total, money,
      ...
      <div className="space-y-8">
        ...
        <div className="relative flex items-center max-w-xl mx-auto">
          ...
          <input
            ...
            style={{ background: C.elevated, border: '1px solid var(--color-vel-border-gold)' }}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredCars.map((c, i) => (
              <motion.button
                ...
                style={{ background: C.elevated, border: '1px solid var(--color-border-soft)' }}
                onMouseEnter={e => {
                  (e.currentTarget as HTMLElement).style.borderColor = 'rgba(212,175,55,0.28)';
                  (e.currentTarget as HTMLElement).style.boxShadow = '0 0 30px var(--color-gold-soft)';
                }}
                onMouseLeave={e => {
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="w-8 h-8 rounded-xl flex items-center justify-center"
                      style={{ background: 'rgba(212,175,55,0.09)', border: '1px solid var(--color-vel-border-gold)' }}>
          ...
          <p className="text-vel-muted text-sm">
            {car.registration} · {promo
              ? <>
                  ...
                  <span className="font-bold" style={{ color: C.accent }}>{money(promo.newPrice)}/j</span>
                </>
              : <>{money(car.priceDay)}/j</>}
          </p>
        </div>
        <button
      ...
      <SectionCard>
        ...
        {days > 0 && (
          <motion.div
            ...
            style={{ background: 'var(--color-gold-soft)', border: '1px solid var(--color-gold-soft)' }}
          >
            <span className="text-xl">📅</span>
            <span className="text-vel-slate font-bold text-sm">
              {days} {{ fr: 'jour(s)', ar: 'يوم' }[lang]} ·{' '}
              <span style={{ color: C.accent }}>{money(total)}</span>
              {promo && (
                <span className="ml-2 text-xs px-2 py-0.5 rounded font-bold text-white" style={{ background: 'var(--color-act-delete)' }}>
